...
class DepartmentManager(DatafactzModel):
    ...
    objects = DatafactzCompanyManager("manager__employee_work_info__company_id")
...
class TicketType(DatafactzModel):
...
class ClaimRequest(DatafactzModel):
...
class Attachment(DatafactzModel):
...
class FAQCategory(DatafactzModel):
