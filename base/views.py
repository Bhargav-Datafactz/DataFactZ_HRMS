...
from datafactz.datafactz_settings import (
    APPS,
    DB_INIT_PASSWORD,
    DYNAMIC_URL_PATTERNS,
    FILE_STORAGE,
)
from datafactz.methods import get_datafactz_model_class, remove_dynamic_url
from datafactz_audit.forms import HistoryTrackingFieldsForm
from datafactz_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        if request.method == "POST":
            ...
            from datafactz.datafactz_settings import DB_INIT_PASSWORD as db_password
        ...
        return render(request, "initialize_database/datafactz_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/datafactz_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/datafactz_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/datafactz_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/datafactz_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/datafactz_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/datafactz_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/datafactz_job_position.html",
...
class DatafactzPasswordResetView(PasswordResetView):
    """
    Datafactz View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    Datafactz View for Employee Reset Password
    """
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_datafactz_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from DataFactZ")
                    ...
                    <table align="center" width="600" cellpadding="0" cellspacing="0" border="0" style="border: 1px solid #e0e0e0; border-radius: 10px; overflow: hidden;">
                        <tr>
                            <td align="center" bgcolor="#4CAF50" style="padding: 20px 0;">
                                <h1 style="color: #ffffff; margin: 0;">DataFactz</h1>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding: 20px;">
                                ...
                                <b><p style="font-size: 14px;">Hi,<br>
                                    This email is being sent as part of mail sever testing from DataFactZ.</p></b>
                        ...
                        <tr>
                            <td bgcolor="#f0f0f0" style="padding: 10px; text-align: center;">
                                <p style="font-size: 12px; color: black;">&copy; 2024 DataFactZ, Inc.</p>
                            </td>
                        </tr>
                    </table>
                </body>
            ...
            try:
                ...
                image_path = path.join(
                    settings.STATIC_ROOT, "images/ui/datafactz-logo.png"
                )
...
@login_required
@permission_required("base.view_datafactzmailtemplate")
...
@login_required
@hx_request_required
@permission_required("base.change_datafactzmailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = DatafactzMailTemplate.objects.get(id=obj_id)
...
@login_required
@require_http_methods(["POST"])
@permission_required("base.add_datafactzmailtemplate")
...
@login_required
@permission_required("base.delete_datafactzmailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = DatafactzMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_datafactz_model_class(app_label="attendance", model="gracetime")
...
def employee_permission_assign(request):
    ...
    installed_apps = [app for app in settings.INSTALLED_APPS if app in datafactz_apps]
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_datafactz_model_class(
...
@login_required
@permission_required("datafactz_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("datafactz_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("datafactz_audit.change_audittag")
...
@login_required
def get_datafactz_installed_apps(request):
