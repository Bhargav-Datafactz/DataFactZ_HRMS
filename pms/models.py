...
from datafactz_automations.methods.methods import get_model_class
from datafactz_views.cbv_methods import render_template

"""Objectives and key result section"""


class Period(DatafactzModel):
...
class KeyResult(DatafactzModel):
    ...
    history = DatafactzAuditLog(bases=[DatafactzAuditInfo])
...
class Objective(DatafactzModel):
...
class EmployeeObjective(DatafactzModel):
    ...
    history = DatafactzAuditLog(bases=[DatafactzAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
    objects = DatafactzCompanyManager("employee_id__employee_work_info__company_id")
...
class Comment(models.Model):
    ...
    history = DatafactzAuditLog(excluded_fields=["comment"], bases=[DatafactzAuditInfo])
    objects = DatafactzCompanyManager(
...
class QuestionTemplate(DatafactzModel):
...
class Question(DatafactzModel):
    ...
    objects = DatafactzCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(DatafactzModel):
    ...
    objects = DatafactzCompanyManager("question_id__template_id__company_id")


class Feedback(DatafactzModel):
...
class EmployeeBonusPoint(DatafactzModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
